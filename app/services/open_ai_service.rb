...
class OpenAiService
  JOB_DESC = <<~DESC
    Job Opportunity at Plaza: Key Information
    Company Overview:
    Name: Plaza
  ...
  def make_api_call(message)
    @client.chat(
      parameters: {
        model: "gpt-3.5-turbo",
        messages: [
          { role: 'system', content: "You are a Recruiter called Fernanda. Answer questions using the job description: #{JOB_DESC} after each response, ask if there are any other questions." },
  ...
  def handle_error(error)
    Rails.logger.error "OpenAI API call failed: #{error.message}"
